...
module.exports.profilePageController = async (req, res) => {
    try {
        ...
        let sortOrder = byDate === "1" ? 1 : -1; 

        let hisaabs = await hisaabModel.find(filter).sort({ createdAt: sortOrder });

        res.render("profile", { message: req.flash(), user, query: req.query, hisaabs });
    } catch (err) {
        req.flash("error", err.message);
        return res.redirect('/');
    }
